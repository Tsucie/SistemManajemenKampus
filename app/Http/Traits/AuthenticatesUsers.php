...
use App\Models\User as UserContext;
...
trait AuthenticatesUsers
{
    ...
    public function login(Request $request)
    {
        ...
        if (Auth::guard('web')->check()) {
            ...
            UserContext::query()->where('u_username','=','@'.$request->u_username)->update($update);
    ...
    {
        $data = [
            $this->username() => '@'.$request->u_username,
    ...
    {
        $request->validate(['u_id' => 'required']);

        $this->guard()->logout();
        
        //Update login status
        $update = [
            'u_logout_time' => DateTime::Now(),
            'u_login_status' => 0
        ];
        UserContext::query()->where('u_id','=',$request->u_id)->update($update);
